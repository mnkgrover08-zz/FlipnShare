		...
		<?php
			...
			//echo '<pre>';print_r($itemObject);
				if($count !=0) {
					...
					$mediaURL = '/'.$projectFolder.$itemObject->imageorvideo[0]->file;
					if(isset($itemObject->imageorvideo[0]->file)) {
						echo '<div class="potrait-full nowrap" style="height:100%;background-image:url('.$mediaURL.');background-size:cover;background-position:center;"></div><div class="interview">';
						echo '<div class="title">'.$articleJson->heading.'</div>';
						echo '<div class="authorDetails">'.$articleJson->authorDetails.'</div>';
						echo '<div class="summary">'.$articleJson->summary.'</div>';
						echo '<div class="item-title">'.$itemObject->headline.'</div><div class="item-description">'.$itemObject->description.'</div></div>';
					}
					else {
						echo '<div class="item-noimg nowrap"><div class="item-title">'.$itemObject->headline.'</div><div class="item-description">'.$itemObject->description.'</div></div>';
					}					
...
$('.page-half').css('width', inWidth*4/5);
